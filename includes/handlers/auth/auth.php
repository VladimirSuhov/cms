        ...
        if (!empty( $_POST['login'] ) &&
            ...
            if ( $_SESSION['captcha'] == md5($captha) ) {
                ...
                if ( $row['password'] !== $password ) {
                    echo json_encode( array( 'succes' => 'false', 'error' => 'Incorrect password' ) );
                } elseif ( (int) $row['active'] !== 1 ) {
                    echo json_encode( array( 'succes' => 'false', 'error' => 'Account not active' ) );
                } else {
...
if ( $Module == 'reset' && $_POST['enter'] ) {
    if (!empty( $_POST['email'] ) ) {
        ...
        if ( $_SESSION['captcha'] == md5($captha) ) {
            ...
            if (! $row['email'] ) {
                echo json_encode( array( 'success' => 'false', 'message' => 'There is no user with this email' ) );
            } else {
                if ( ! empty($_SESSION['PASS_RESTORE'] ) ) {
                    ...
                    echo json_encode(array('success' => 'true', 'message' => 'Mail with the confirm link has been sended on your email'));
                }
            }
        }
    }
